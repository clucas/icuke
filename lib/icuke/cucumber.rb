...
class ICukeWorld
  ...
  def type(textfield, text, options = {})
    ...
    if options[:x] && options[:y]
      element = screen.first_tappable_element(textfield)
      x, y = screen.element_center(element)
      x = x + options[:x].to_s.to_f
      y = y + options[:y].to_s.to_f
      @simulator.fire_event(Tap.new(x, y, options))
      sleep(options[:pause] ? 2 : 0.2)
      refresh
    end  
    ...
    text.split('').each do |c|
      begin
        if c == '.'
          tap(['more, numbers'].shift, :pause => false)
        end  
    ...
    return_keys = ['done']#, 'return', 'go', 'google', 'join', 'next', 'route', 'search', 'send', 'yahoo', 'emergency call']
...
When /^I tap "([^\"]*)" and wait (\d+)$/ do |label, pause|
  tap(label, {:pause => pause})
  sleep(pause.to_s.to_i)
end
...
When /^I type "([^\"]*)" in "([^\"]*)" with x\+(\d+) pixels and y\+(\d+) pixels$/ do |text, textfield, x, y|
  type(textfield, text, {:x => x, :y => y})
end
